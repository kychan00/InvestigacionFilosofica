...
if SHARD_PARTS < 1:
    raise ValueError(
        "OPENALEX_SHARD_PARTS debe ser >= 1"
    )

if not (
    0 <= SHARD_PART < SHARD_PARTS
):
    raise ValueError(
        "OPENALEX_SHARD_PART fuera de rango"
    )
...
OUTPUT = Path(
    ...
    f"v2-part-{SHARD_PART:02d}.parquet"
)
...
print("=" * 78)
print("OPENALEX PHILOSOPHY CLASSIFIER V2")
print("=" * 78)
...
# CLASSIFIER V2
#
# La auditoría de independencia demostró que la keyword
# correspondiente al concepto ancla replica exactamente
# concept_score. Por tanto:
#
#   concept anchor              = una señal
#   keyword del mismo anchor    = NO suma
#   generic "philosophy"        = corroboración no duplicada
#   otra keyword *philosoph*    = corroboración no duplicada
#
# Los conceptos múltiples siguen siendo evidencia adicional,
# aunque procedan también de OpenAlex.
# ============================================================
...
ambiguous_ids_v2 = [
    ...
    if name in ambiguous_names_v2
]


ambiguous_ids_sql_v2 = (
    ", ".join(
        str(value)
...
# ------------------------------------------------------------
# Lookup concepto -> slug de SU propia keyword.
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Buscar keywords filosóficas que NO sean:
#
#   1. la keyword duplicada del concepto principal
#   2. la keyword genérica "philosophy"
#
# La genérica ya está disponible por separado en
# philosophy_keyword_score.
# ------------------------------------------------------------
...
con.execute(
    ...
    JOIN v2_concept_lookup l
        ON l.concept_id =
           c.primary_concept_id

    LEFT JOIN candidate_keywords k
        ON k.work_id =
           c.work_id
...
#
# CORE:
#   requiere corroboración no duplicada fuerte.
#
# PROBABLE:
#   alguna corroboración,
#   varios conceptos,
#   o concepto fuerte no altamente ambiguo.
#
# BORDERLINE:
#   mantiene recall y será el grupo ideal para abstracts.
#
# EXCLUDE:
#   sólo colisión de Phenomenology con hard science
#   ya demostrada repetidamente.
# ------------------------------------------------------------
...
con.execute(
    ...
    WITH evidence AS (
        ...
        SELECT
            ...
            s.secondary_philosophy_score,

            coalesce(
                s.secondary_philosophy_count,
                0
            )
                AS secondary_philosophy_count,
            ...
            greatest(
                ...
                AS nonduplicate_philosophy_score


        FROM classified c

        LEFT JOIN v2_secondary_keywords s
            ON s.work_id =
               c.work_id
    )
    ...
    SELECT
        ...
        e.primary_concept_id,
        e.concept_score,
        e.philosophy_concept_count,

        e.keyword_count,


        e.philosophy_keyword_score
            AS generic_philosophy_score,

        e.secondary_philosophy_score,
        e.secondary_philosophy_count,

        e.nonduplicate_philosophy_score,
        ...
        e.top_domain_keyword_id,
        e.top_domain_keyword_score,

        e.hard_noise_score,
        e.hard_noise_keyword,
        ...
        CASE
            ...
            -- CORE
            --
            -- Ya no se permite que la keyword del propio
            -- concepto cree artificialmente una segunda señal.
            -- ================================================
            ...
            -- Score alto por sí solo puede hacer una obra
            -- PROBABLE, pero NO si el concepto es uno de los
            -- dos casos con colisiones léxicas demostradas.
        ...
        CASE
            ...
            WHEN
                ...
                THEN
                'strong-nonduplicate-corroboration'
            ...
            WHEN
                ...
                THEN
                'generic-plus-secondary-philosophy'
            ...
            WHEN
                ...
                THEN
                'corroboration-plus-multiple-concepts'
            ...
            WHEN
                ...
                THEN
                'moderate-nonduplicate-corroboration'
...
for tier, count in v2_rows:

    print(
        f"{tier:12} "
        f"{int(count):8,}  "
        f"{int(count) / max(v2_total, 1) * 100:5.1f}%"
    )


print()
print("=" * 78)
print("TRANSICIÓN V1 → V2")
print("=" * 78)
...
for old_tier, new_tier, count in transition_rows:

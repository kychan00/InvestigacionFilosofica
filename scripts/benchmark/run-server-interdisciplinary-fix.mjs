...
const PORT = Number(process.env.BENCHMARK_PORT || 4176);
...
const runName =
  `human-v${benchmarkVersion}-interdisciplinary-expansionfix-` +
...
async function handleApi(request, response, pathname) {
  if (request.method === "POST" && pathname === "/__benchmark/start") {
    if (fs.existsSync(finalPath)) {
      sendJson(response, 409, {
        error: "Frozen interdisciplinary expansion-fix run already exists.",
  ...
  if (request.method === "POST" && pathname === "/__benchmark/finalize") {
    ...
    const meta = {
      ...
      profile: {
        name: "experimental-interdisciplinary-expansion-fix",
        baseProductionRuntime: manifest.release.runtimeCommit,
        rankingPolicy: "ranking-v2-current",
        retrievalChange: "semantic expansion safety: do not drop substantive residual query constraints",
        maxQueries: 5,
...
function serveStatic(response, pathname) {
  let requested = pathname === "/"
    ? "/benchmark/runner-interdisciplinary-fix.html"
    : pathname;
...
server.listen(PORT, "127.0.0.1", () => {
  console.log();
  console.log("INTERDISCIPLINARY EXPANSION-FIX BENCHMARK SERVER");
  console.log("==============================================");
  ...
  console.log();
  console.log(`Open: http://127.0.0.1:${PORT}/benchmark/runner-interdisciplinary-fix.html`);
  console.log();

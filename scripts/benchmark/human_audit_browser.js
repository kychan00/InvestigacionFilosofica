...
async function saveCurrent(relevance = undefined, { advance = false } = {}) {
  ...
  try {
    ...
    await requestJson("/__human-audit/save", {
      method: "POST",
...
nextUnlabeled.addEventListener("click", () => {
  ...
  if (target === null) {
    status.textContent = "Ya no quedan documentos sin juzgar.";
    return;
  }
  index = target;
  render();
...
finalize.addEventListener("click", async () => {
  ...
  try {
    const result = await requestJson("/__human-audit/finalize", {
      method: "POST",
      body: "{}",
    });

    finalStatus.textContent = `Auditoría finalizada: ${result.rows} juicios guardados en ${result.output}`;
    status.textContent = "Auditoría finalizada correctamente.";
  } catch (error) {
...
async function initialize() {
  try {
    const state = await requestJson("/__human-audit/state", {
      headers: {},
    });
    ...
    if (state.finalized) {
      finalStatus.textContent = `Esta auditoría ya fue finalizada${state.finalizedAt ? ` el ${state.finalizedAt}` : ""}.`;
    }
  } catch (error) {

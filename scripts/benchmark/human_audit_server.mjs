...
const PORT = Number(process.env.HUMAN_AUDIT_PORT || 4175);

const SAMPLE_PATH = path.join(ROOT, "benchmark/human-audit-v1.sample.jsonl");
const PROGRESS_PATH = path.join(ROOT, "benchmark/.human-audit-v1.progress.json");
const FINAL_PATH = path.join(ROOT, "benchmark/human-audit-v1.judgments.jsonl");
...
if (!fs.existsSync(SAMPLE_PATH)) {
  console.error("Missing benchmark/human-audit-v1.sample.jsonl");
  console.error("Run: npm run benchmark:human-audit:sample");
  process.exit(1);
}

const sample = readJsonl(SAMPLE_PATH);
if (sample.length !== 100) {
  console.error(`Expected 100 audit rows, found ${sample.length}`);
  process.exit(1);
}
  ...
  if (request.method === "POST" && pathname === "/__human-audit/finalize") {
    ...
    if (missing.length) {
      sendJson(response, 409, {
        error: "Human audit is incomplete.",
        missing,
      });
      return true;
    }
    ...
    sendJson(response, 200, {
      status: "finalized",
      rows: rows.length,
      output: "benchmark/human-audit-v1.judgments.jsonl",
      finalizedAt,
    });
    return true;
  }
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (!handled) serveStatic(response, url.pathname);
  } catch (error) {
    console.error(error);
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  console.log();
  console.log("HUMAN AUDIT SERVER");
  console.log("==================");
  console.log(`Progress: ${state.labeled}/${state.total}`);
  console.log(`Open: http://127.0.0.1:${PORT}/benchmark/human-audit.html`);
  console.log();

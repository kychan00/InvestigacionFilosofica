...
const PORT = Number(process.env.MULTILINGUAL_DELTA_AUDIT_PORT || 4179);

const SAMPLE_PATH = path.join(ROOT, "benchmark/multilingual-constraints-delta-audit-v1.sample.jsonl");
const PROGRESS_PATH = path.join(ROOT, "benchmark/.multilingual-constraints-delta-audit-v1.progress.json");
const FINAL_PATH = path.join(ROOT, "benchmark/multilingual-constraints-delta-audit-v1.judgments.jsonl");
...
if (!fs.existsSync(SAMPLE_PATH)) {
  console.error("Missing benchmark/multilingual-constraints-delta-audit-v1.sample.jsonl");
  console.error("Run: npm run benchmark:multilingual-delta-audit:sample");
  process.exit(1);
}

const sample = readJsonl(SAMPLE_PATH);
if (!sample.length) {
  console.error("Multilingual delta audit sample is empty; there is nothing to judge.");
  process.exit(1);
}
...
async function handleApi(request, response, pathname) {
  ...
  if (request.method === "POST" && pathname === "/__human-audit/finalize") {
    ...
    sendJson(response, 200, {
      status: "finalized",
      rows: rows.length,
      output: "benchmark/multilingual-constraints-delta-audit-v1.judgments.jsonl",
      finalizedAt,
    });
    return true;
  }
...
server.listen(PORT, "127.0.0.1", () => {
  const state = statePayload();
  console.log();
  console.log("MULTILINGUAL CONSTRAINTS DELTA AUDIT SERVER");
  console.log("===========================================");
  ...
  console.log();
  console.log("Blindness: old/new side, ranks, providers, and translation provenance are hidden.");

...
const PORT = Number(process.env.BENCHMARK_PORT || 4178);
...
const targetQueries = benchmark.queries.filter(
  query => query.intent === "interdisciplinary-challenge"
);
...
const runName =
  `human-v${benchmarkVersion}-interdisciplinary-multilingual-` +
  runtimeCommit.slice(0, 7);

const comparisonRun =
  "benchmark/runs/human-v1.0-interdisciplinary-expansionfix-cfb853a.jsonl";
...
const expectedQueries = new Set(targetQueries.map(query => query.id));
...
function validateRows(queryId, rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (!Array.isArray(row.matchedQueries)) {
      throw new Error(`${queryId}: matchedQueries provenance missing`);
    }
...
async function handleApi(request, response, pathname) {
  if (request.method === "POST" && pathname === "/__benchmark/start") {
    if (fs.existsSync(finalPath)) {
      sendJson(response, 409, {
        error: "Frozen multilingual-constraints run already exists.",
    ...
    sendJson(response, 200, {
      ...
      rankingDepth,
      poolDepth,
      expectedQueries: targetQueries.length,
  ...
  if (request.method === "POST" && pathname === "/__benchmark/query") {
    ...
    if (!expectedQueries.has(queryId)) {
      sendJson(response, 400, { error: `Unknown interdisciplinary query: ${queryId}` });
      return true;
    }
  ...
  if (request.method === "POST" && pathname === "/__benchmark/finalize") {
    ...
    const missing = targetQueries
      .map(query => query.id)
      .filter(id => !completed.has(id));

    if (missing.length) {
      sendJson(response, 409, { error: "Focused benchmark is incomplete.", missing });
      return true;
    }
    ...
    const meta = {
      ...
      queryFilter: { intent: "interdisciplinary-challenge" },
      rankingDepth,
      poolDepth,
      rows: allRows.length,
      perQuery,
      matchedQueriesSerialized: true,
      comparisonRun,
      profile: {
        name: "experimental-multilingual-constraints-v1",
        baseProductionRuntime: manifest.release.runtimeCommit,
        rankingPolicy: "ranking-v2-current",
        retrievalChange:
          "multilingual academic-domain recognition plus safe complete-English expansion that preserves recognized and unknown constraints",
        supportedQueryLanguages: ["es", "en", "de", "fr", "pt"],
        maxQueries: 5,
...
function serveStatic(response, pathname) {
  let requested = pathname === "/"
    ? "/benchmark/runner-multilingual-constraints.html"
    : pathname;
...
server.listen(PORT, "127.0.0.1", () => {
  console.log();
  console.log("MULTILINGUAL CONSTRAINTS BENCHMARK SERVER");
  console.log("=========================================");
  ...
  console.log(`Comparison run: ${comparisonRun}`);
  console.log(`Queries: ${targetQueries.length}`);
  console.log(`Pool depth: ${poolDepth}`);
  console.log("matchedQueries provenance: required");
  console.log();
  console.log(`Open: http://127.0.0.1:${PORT}/benchmark/runner-multilingual-constraints.html`);
  console.log();

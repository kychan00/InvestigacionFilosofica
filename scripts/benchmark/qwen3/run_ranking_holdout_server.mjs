...
const PREREG_PATH = 'benchmark/qwen3/ranking/validation/qwen3-ranking-holdout-v1.preregistered.json';
const QUERIES_PATH = 'benchmark/qwen3/ranking/validation/qwen3-ranking-holdout-v1.queries.json';
...
const runName = `${prereg.validation_id}-production-pool-${runtimeCommit.slice(0, 7)}`;
const runDirectory = path.join(ROOT, 'benchmark/qwen3/ranking/validation/runs');
...
async function handleApi(request, response, pathname) {
  if (request.method === 'POST' && pathname === '/__qwen3_ranking_holdout/start') {
    if (fs.existsSync(finalPath)) {
      sendJson(response, 409, {
        error: 'Frozen Qwen3 ranking holdout production pool already exists.',
  ...
  if (request.method === 'POST' && pathname === '/__qwen3_ranking_holdout/query') {
  ...
  if (request.method === 'POST' && pathname === '/__qwen3_ranking_holdout/finalize') {
    ...
    const meta = {
      schema_version: 'qwen3-ranking-holdout-production-pool-meta-v1',
      ...
      note: 'Fresh ranking holdout retrieval using the existing production search code. Qwen and human labels are excluded from retrieval.',
    };
...
function serveStatic(response, pathname) {
  let requested = pathname === '/' ? '/benchmark/qwen3/runner-ranking-holdout.html' : pathname;
  try {
...
server.listen(PORT, '127.0.0.1', () => {
  console.log();
  console.log('QWEN3 RANKING HOLDOUT · FRESH PRODUCTION POOL');
  console.log('==============================================');
  ...
  console.log(`Open: http://127.0.0.1:${PORT}/benchmark/qwen3/runner-ranking-holdout.html`);

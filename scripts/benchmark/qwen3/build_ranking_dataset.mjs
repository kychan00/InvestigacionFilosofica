...
const SOURCE = 'benchmark/qwen3/ranking/runs/qwen3-ranking-v1-production-pool-c2ca5ed.jsonl';
const OUTPUT = 'benchmark/qwen3/ranking/datasets/qwen3-ranking-v1.jsonl';
const META = 'benchmark/qwen3/ranking/datasets/qwen3-ranking-v1.meta.json';
const SOURCE_SHA256 = '596dcb74f786a4e4ff40a705ec9f440accdfebca1ffcdadcdad2548ef7003b4c';
const SOURCE_FREEZE_COMMIT = '36314cc';
...
function validateFrozenPool(records) {
  if (records.length !== EXPECTED_ROWS) {
    throw new Error(`ranking pool row mismatch: expected ${EXPECTED_ROWS}, got ${records.length}`);
  }
  ...
  if (byQuery.size !== EXPECTED_QUERIES) {
    throw new Error(`ranking pool query mismatch: expected ${EXPECTED_QUERIES}, got ${byQuery.size}`);
  }
  ...
  for (const [queryId, rows] of byQuery) {
    ...
    if (pairIds.size !== EXPECTED_PER_QUERY) {
      throw new Error(`${queryId}: duplicate record_id in frozen pool`);
    }
  }
}
...
export async function buildRankingDataset() {
  ...
  if (actualSourceSha !== SOURCE_SHA256) {
    throw new Error(`ranking pool SHA mismatch: expected ${SOURCE_SHA256}, got ${actualSourceSha}`);
  }
  ...
  if (records.length !== EXPECTED_ROWS) {
    throw new Error(`Qwen dataset row mismatch: expected ${EXPECTED_ROWS}, got ${records.length}`);
  }
  if (stats.duplicate_rows_removed !== 0) {
    throw new Error(`Qwen ranking dataset unexpectedly removed ${stats.duplicate_rows_removed} duplicate rows`);
  }
  if (stats.unique_queries !== EXPECTED_QUERIES) {
    throw new Error(`Qwen dataset query mismatch: expected ${EXPECTED_QUERIES}, got ${stats.unique_queries}`);
  }
  ...
  const metadata = {
    schema_version: 'qwen3-ranking-dataset-meta-v1',
    dataset_id: 'qwen3-ranking-v1-model-input',
    purpose: 'development-ranking-model-input',
...
async function main() {
  ...
  console.log('Qwen3 ranking dataset build complete');

...
function blockNumberForIndex(value) {
  return Math.floor(value / blockSize) + 1;
}

function blockRange(blockNumber) {
  const start = (blockNumber - 1) * blockSize;
  const end = Math.min(start + blockSize, rows.length);
  return { start, end };
}

function labeledInBlock(blockNumber) {
  const { start, end } = blockRange(blockNumber);
...
function renderProgress() {
  ...
  progress.max = rows.length || 160;
  counter.textContent = `${labeled}/${rows.length || 160} etiquetados`;
  donePanel.style.display = labeled === rows.length && rows.length ? "block" : "none";

  for (const button of blockButtons) {
    const blockNumber = Number(button.dataset.block);
    const { start, end } = blockRange(blockNumber);
    button.textContent = `Bloque ${blockNumber} · ${labeledInBlock(blockNumber)}/${end - start}`;
    button.classList.toggle("selected", blockNumberForIndex(index) === blockNumber);
  }
}
...
function render() {
  ...
  const blockNumber = blockNumberForIndex(index);
  const { start, end } = blockRange(blockNumber);

  auditId.textContent = row.audit_id;
  position.textContent = `${index + 1} de ${rows.length}`;
  blockInfo.textContent = `Bloque ${blockNumber} · QRH${String(start + 1).padStart(3, "0")}–QRH${String(end).padStart(3, "0")}`;
...
async function saveCurrent(relevance = undefined, { advance = false } = {}) {
  ...
  try {
    await requestJson("/__qwen3_ranking_holdout_audit/save", {
      method: "POST",
      body: JSON.stringify({
...
for (const button of blockButtons) {
  button.addEventListener("click", () => {
    ...
    for (let candidate = start; candidate < end; candidate += 1) {
      if (!Number.isInteger(judgments[rows[candidate].audit_id]?.human_relevance)) {
        target = candidate;
        break;
      }
    }
...
note.addEventListener("input", () => {
  ...
  noteTimer = setTimeout(() => saveCurrent(undefined, { advance: false }), 650);
});
...
finalize.addEventListener("click", async () => {
  ...
  try {
    const result = await requestJson("/__qwen3_ranking_holdout_audit/finalize", {
      method: "POST",
      body: "{}",
    });
    finalStatus.textContent = `Holdout de ranking finalizado: ${result.rows} juicios guardados.`;
...
async function initialize() {
  try {
    const state = await requestJson("/__qwen3_ranking_holdout_audit/state", { headers: {} });
    rows = state.rows;
    judgments = state.judgments || {};
    blockSize = state.blockSize || 40;

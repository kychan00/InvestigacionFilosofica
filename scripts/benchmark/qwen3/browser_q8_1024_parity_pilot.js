...
function assertSessionContract(model) {
  ...
  for (const [name, session] of entries) {
    ...
    if (session.config?.dtype !== 'q8') {
      throw new Error(`session ${name} did not select q8`);
    }
  }
  return configs;
}
...
function restrictModelSessionOutputsToLogits(model) {
  ...
  session.run = function runLogitsOnly(feeds, fetches, options) {
    ...
    // prevents those unused outputs from being allocated at the frozen token boundary.
...
async function createScorer(payload, gpuAdapter) {
  ...
  // avoid atomic handle-release faults at the frozen token boundary.
  ...
  return {
    ...
    async score(row) {
      ...
      if (ids.length > prereg.prompt_contract.max_length) throw new Error(`tokenized input exceeds frozen max length ${prereg.prompt_contract.max_length}`);
      ...
      // and avoids a multi-gigabyte [sequence, vocabulary] output at the frozen token boundary.
...
async function main() {
  ...
  document.title = 'qwen3-browser-q8-1024-parity-pilot-v1 complete';
}
...
main().catch(async (error) => {
  ...
  document.title = 'qwen3-browser-q8-1024-parity-pilot-v1 failed';
});

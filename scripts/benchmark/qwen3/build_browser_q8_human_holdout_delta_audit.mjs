...
const AB_PATH = 'benchmark/qwen3/browser/q8-human-holdout/runs/qwen3-browser-q8-human-holdout-v1-ab.jsonl';
const AB_META_PATH = 'benchmark/qwen3/browser/q8-human-holdout/runs/qwen3-browser-q8-human-holdout-v1-ab.meta.json';
const AB_SHA256 = '002ef734a26b7a7f7fc932422f4a4187f0bbb2bddab460df253dbddd1b0b196d';
const AB_META_SHA256 = '3225b67c62246ebc9697d30e4a278356ea45465f133676c89cfb305290b67f14';
const AB_FREEZE_COMMIT = '51d9106c45fd8fb5b2ba8d547b87f5f4f960e618';

const SAMPLE_PATH = 'benchmark/qwen3/browser/q8-human-holdout/audit/qwen3-browser-q8-human-holdout-v1-delta-audit.sample.jsonl';
const SAMPLE_META_PATH = 'benchmark/qwen3/browser/q8-human-holdout/audit/qwen3-browser-q8-human-holdout-v1-delta-audit.sample.meta.json';
const WORKSHEET_PATH = 'benchmark/qwen3/browser/q8-human-holdout/audit/qwen3-browser-q8-human-holdout-v1-delta-audit.sample.txt';

const AUDIT_ID_PREFIX = 'Q8H';
const AUDIT_VERSION = 'qwen3-browser-q8-human-holdout-v1-delta-audit-v1';
const EXPECTED_QUERIES = 25;
const EXPECTED_CHANGED_PAIRS = 192;
const EXPECTED_SIDE_COUNT = 96;
...
function renderWorksheet(sample, metadata) {
  const header = [
    'AUDITORÍA HUMANA CIEGA · QWEN3 BROWSER Q8 HUMAN HOLDOUT V1',
    '',
...
function assertOutputsAbsent() {
  for (const path of [SAMPLE_PATH, SAMPLE_META_PATH, WORKSHEET_PATH]) {
    if (existsSync(resolve(path))) throw new Error(`refusing to overwrite blind audit output: ${path}`);
  }
}

export async function buildBrowserQ8HumanHoldoutDeltaAudit({ writeOutput = false } = {}) {
  assertOutputsAbsent();
  ...
  const sampleMeta = {
    schema_version: 'qwen3-browser-q8-human-holdout-delta-audit-sample-meta-v1',
    ...
    contains_browser_q8_score: false,
...
async function main() {
  ...
  const result = await buildBrowserQ8HumanHoldoutDeltaAudit({
    writeOutput: mode === 'run',
  });

  console.log(JSON.stringify({
    experiment_id: 'qwen3-browser-q8-human-holdout-v1',
    status: mode === 'preflight' ? 'preflight-passed-no-output' : 'blind-audit-built',
    audit_rows: result.sampleMeta.row_count,
    queries: result.sampleMeta.query_count,
    expected_A_only_hidden: EXPECTED_SIDE_COUNT,
    expected_B_only_hidden: EXPECTED_SIDE_COUNT,
    private_mapping_artifact_written: false,
    sample_sha256: result.hashes.sample,
    sample_meta_sha256: result.hashes.sample_meta,
    worksheet_sha256: result.hashes.worksheet,
    output_written: mode === 'run',
    contains_condition: false,
    contains_rank: false,
    contains_query_id: false,
    contains_record_id: false,

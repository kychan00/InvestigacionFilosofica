...
async function run() {
  ...
  try {
    status.textContent = 'Cargando preregistro, consultas frescas y mapa…';
    const [preregResponse, queriesResponse, mapResponse] = await Promise.all([
      fetch('/benchmark/qwen3/browser/q8-human-holdout/qwen3-browser-q8-human-holdout-v1.preregistered.json', { cache: 'no-store' }),
      fetch('/benchmark/qwen3/browser/q8-human-holdout/qwen3-browser-q8-human-holdout-v1.queries.json', { cache: 'no-store' }),
    ...
    log(`queries=${querySet.queries.length}`);
    ...
    for (let index = 0; index < querySet.queries.length; index += 1) {
      ...
      status.textContent = `${index + 1}/${querySet.queries.length}\n${query.id}\n${query.query}`;
      ...
      await postJson('/__qwen3_browser_q8_human_holdout/query', {
        queryId: query.id,
        rows,
    ...
    status.textContent = 'Finalizando pool fresco…';
    const final = await postJson('/__qwen3_browser_q8_human_holdout/finalize', {});
    progress.value = querySet.queries.length;
    status.textContent = `Q8 HUMAN HOLDOUT · POOL CONGELADO\n${final.run}`;

...
} from './browser_q8_1024_parity_pilot_core.mjs';

const projectRoot = resolve(dirname(fileURLToPath(import.meta.url)), '../../..');
const preregistrationPath = 'benchmark/qwen3/browser/q8-1024-parity-pilot/qwen3-browser-q8-1024-parity-pilot-v1.preregistered.json';
const browserEntryPath = 'scripts/benchmark/qwen3/browser_q8_1024_parity_pilot.js';
const expectedPreregSha256 = '5c4fd21aa9414048fe6c045350a773d661e316c902c13a225cc28ba5a976a79b';
...
function parseArgs(argv) {
  ...
  for (const argument of argv) {
    if (argument === '--preflight') options.mode = 'preflight';
      ...
      throw new Error('diagnostic mode is forbidden for the preregistered pilot');
...
async function readFrozenInputs() {
  ...
  if (preregSha256 !== expectedPreregSha256) {
    throw new Error(`preregistration SHA-256 mismatch: ${preregSha256}`);
  }
  ...
  const referencePath = resolve(projectRoot, prereg.reference_model.pilot_reference_path);
  const referenceBytes = await readFile(referencePath);
  const referenceSha256 = sha256Bytes(referenceBytes);
  if (referenceSha256 !== prereg.reference_model.pilot_reference_sha256) {
...
function assertRuntimeMetadata(metadata) {
  if (
    ...
    throw new Error('browser runtime metadata violates the frozen WebGPU/q8 contract');
  }
  ...
  if (configs.length === 0 || configs.some((config) => config?.device !== 'webgpu' || config?.dtype !== 'q8')) {
...
async function runExperiment({
  ...
  if (mode !== 'run') {
    throw new Error('only the preregistered official --run mode may execute browser scoring');
  }
  const frozen = await readFrozenInputs();
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && requestUrl.pathname === '/') {
        ...
        response.end('<!doctype html><meta charset="utf-8"><title>qwen3-browser-q8-1024-parity-pilot-v1</title><script type="module" src="/runner.js"></script>');
        return;
      }
      ...
      if (request.method === 'POST' && requestUrl.pathname === '/complete') {
        ...
        if (scores.length !== frozen.rows.length) throw new Error(`incomplete browser output: ${scores.length}/${frozen.rows.length}`);
        await assertOutputTargetsAreNew();
        ...
        const metadata = {
          schema_version: 'qwen3-browser-q8-1024-parity-pilot-score-meta-v1',
          experiment_id: BROWSER_PARITY_EXPERIMENT_ID,
        ...
        const referenceScores = parseJsonlText(frozen.referenceBytes.toString('utf8'), {
          source: frozen.prereg.reference_model.pilot_reference_path,
        });
        const report = {
          ...buildBrowserParityReport({
            rows: frozen.rows,
          ...
          artifacts: {
            ...
            reference_scores: {
              path: frozen.prereg.reference_model.pilot_reference_path,

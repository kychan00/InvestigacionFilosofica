...
const expectedPreregSha256 =
  "553681d80c38d45bb5337f7ec6553511acb56cf83449ffae250d118e3c0342e1";
...
function validatePrereg(prereg) {
  if (prereg.experiment_id !== "qwen3-browser-q8-512-warm-v1") {
  ...
  if (prereg.candidate.exact_total_tokens !== 512) {
    throw new Error("frozen token length is not 512");
  }
  ...
  if (!cases.every((item) => item.target_total_tokens === 512)) {
    throw new Error("not every run targets exactly 512 tokens");
  }
...
async function runExperiment({ launch }) {
  ...
  async function finalize({ status, error = null }) {
    const successfulRuns = runs.filter(
      (item) =>
        item.constructed_total_tokens === 512 &&
    ...
    try {
      ...
      if (
        ...
        response.end(
          "<!doctype html>" +
            "<meta charset=\"utf-8\">" +
            "<title>qwen3-browser-q8-512-warm-v1</title>" +
      ...
      if (
        ...
        if (body.target_total_tokens !== 512) {
          throw new Error("target token count changed");
        }

        if (body.constructed_total_tokens !== 512) {
          throw new Error("constructed token count is not exactly 512");
        }
        ...
        runs.push({
          ...
          target_total_tokens: 512,
          constructed_total_tokens: 512,
      ...
      if (
        ...
        if (!settled) {
          ...
          const failure = new Error(
            `q8 512 warm experiment failed: ${error.message}`,
          );
  ...
  console.log(`Q8 512 warm runner ready: ${url}`);
  console.log("Runs: first-1 -> warm-1 -> warm-2 -> warm-3 -> warm-4 -> warm-5");
  console.log("All runs use exactly 512 total tokens in one loaded browser/model session.");
...
async function main() {
  ...
  console.log("===== Q8 512 WARM RESULT =====");

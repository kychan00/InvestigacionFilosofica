...
export const BROWSER_PARITY_EXPERIMENT_ID = 'qwen3-browser-q8-1024-parity-pilot-v1';
export const BROWSER_SCORE_OUTPUT =
  'benchmark/qwen3/browser/q8-1024-parity-pilot/scores/qwen3-browser-q8-1024-parity-pilot-v1.raw.jsonl';
export const BROWSER_SCORE_METADATA_OUTPUT =
  'benchmark/qwen3/browser/q8-1024-parity-pilot/scores/qwen3-browser-q8-1024-parity-pilot-v1.raw.meta.json';
export const BROWSER_PARITY_REPORT_OUTPUT =
  'benchmark/qwen3/browser/q8-1024-parity-pilot/reports/qwen3-browser-q8-1024-parity-pilot-v1.report.json';
export const BROWSER_PARITY_LOG_OUTPUT =
  'benchmark/qwen3/browser/q8-1024-parity-pilot/logs/qwen3-browser-q8-1024-parity-pilot-v1.log';
export const BROWSER_CHECKPOINT =
  'benchmark/qwen3/cache/qwen3-browser-q8-1024-parity-pilot-v1.partial.jsonl';
...
export function assertFrozenBrowserParityPreregistration(prereg) {
  const checks = [
    ...
    [prereg.browser_model.artifact, 'onnx/model_quantized.onnx', 'browser artifact'],
    [prereg.browser_model.dtype, 'q8', 'browser dtype'],
    [prereg.browser_model.device, 'webgpu', 'primary device'],
    [prereg.browser_model.transformers_js_version, '4.3.0', 'Transformers.js version'],
    [prereg.prompt_contract.max_length, 1024, 'max length'],
    ...
    [prereg.selection.rows, 100, 'dataset rows'],
    [prereg.selection.queries, 5, 'dataset queries'],
    [prereg.primary_parity_gate.required_queries_equal, 5, 'parity gate'],
  ];
...
export function validateFrozenDatasetRows(rows, prereg) {
  if (!Array.isArray(rows) || rows.length !== prereg.selection.rows) {
    throw new Error(`dataset must contain exactly ${prereg.selection.rows} rows`);
  }
  ...
  if (counts.size !== prereg.selection.queries) {
    throw new Error(`dataset must contain exactly ${prereg.selection.queries} queries`);
  }
...
export function buildBrowserParityReport({ rows, browserScores, referenceScores, prereg }) {
  ...
  for (const [queryId, reference] of referenceGroups) {
    ...
    const referenceTop5 = reference.slice(0, 5).map((item) => item.record_id);
    const referenceTop10 = reference.slice(0, 10).map((item) => item.record_id);
    const browserTop5 = browser.slice(0, 5).map((item) => item.record_id);
    const browserTop10 = browser.slice(0, 10).map((item) => item.record_id);
    const browserTop5Members = new Set(browserTop5);
    const browserMembers = new Set(browserTop10);
    const top5Overlap = referenceTop5.filter((recordId) => browserTop5Members.has(recordId)).length;
    const overlap = referenceTop10.filter((recordId) => browserMembers.has(recordId)).length;
    perQuery.push({
      query_id: queryId,
      top5_membership_equal: top5Overlap === 5,
  ...
  return {
    schema_version: 'qwen3-browser-q8-1024-parity-pilot-report-v1',
    experiment_id: BROWSER_PARITY_EXPERIMENT_ID,
    primary_gate: {
    ...
    secondary_metrics: {
      exact_top5_membership_queries: perQuery.filter((item) => item.top5_membership_equal).length,

...
async function run() {
  ...
  try {
    ...
    const [preregResponse, queriesResponse, mapResponse] = await Promise.all([
      fetch('/benchmark/qwen3/ranking/validation/qwen3-ranking-holdout-v1.preregistered.json', { cache: 'no-store' }),
      fetch('/benchmark/qwen3/ranking/validation/qwen3-ranking-holdout-v1.queries.json', { cache: 'no-store' }),
    ...
    const session = await postJson('/__qwen3_ranking_holdout/start', {});
    ...
    for (let index = 0; index < querySet.queries.length; index += 1) {
      ...
      await postJson('/__qwen3_ranking_holdout/query', {
        queryId: query.id,
        rows,
    ...
    const final = await postJson('/__qwen3_ranking_holdout/finalize', {});
    progress.value = querySet.queries.length;
    status.textContent = `HOLDOUT POOL CONGELADO\n${final.run}`;

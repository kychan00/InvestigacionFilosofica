...
function renderProgress() {
  ...
  progress.max = rows.length || 192;
  counter.textContent = `${labeled}/${rows.length || 192} etiquetados`;
...
async function saveCurrent(relevance = undefined, { advance = false } = {}) {
  ...
  try {
    await requestJson("/__qwen3_browser_q8_human_holdout_audit/save", {
      method: "POST",
      body: JSON.stringify({
...
finalize.addEventListener("click", async () => {
  ...
  try {
    const result = await requestJson("/__qwen3_browser_q8_human_holdout_audit/finalize", {
      method: "POST",
      body: "{}",
    });
...
async function initialize() {
  try {
    const state = await requestJson("/__qwen3_browser_q8_human_holdout_audit/state", { headers: {} });
    rows = state.rows;

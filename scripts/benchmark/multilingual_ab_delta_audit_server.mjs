...
const PORT = Number(process.env.MULTILINGUAL_AB_AUDIT_PORT || 4181);
const SAMPLE_PATH = path.join(ROOT, "benchmark/multilingual-ab-delta-audit-v1.sample.jsonl");
const PROGRESS_PATH = path.join(ROOT, "benchmark/.multilingual-ab-delta-audit-v1.progress.json");
...
function readProgress() {
  if (!fs.existsSync(PROGRESS_PATH)) return { schemaVersion: 1, updatedAt: null, finalizedAt: null, judgments: {} };
...
if (!fs.existsSync(SAMPLE_PATH)) {
  console.error("Missing benchmark/multilingual-ab-delta-audit-v1.sample.jsonl");
  console.error("Run: npm run benchmark:multilingual-ab-audit:sample");
  process.exit(1);
}

const sample = readJsonl(SAMPLE_PATH);
if (!sample.length) {
  console.error("Audit sample is empty; A and B Top 10 are identical.");
  process.exit(1);
}
...
function statePayload() {
  ...
  const labeled = sample.filter(row => Number.isInteger(judgments[row.audit_id]?.human_relevance)).length;
  return {
    schemaVersion: 1,
    rows: sample,
    judgments,
...
async function handleApi(request, response, pathname) {
  ...
  if (request.method === "POST" && pathname === "/__human-audit/save") {
    ...
    if (relevance !== null && !(Number.isInteger(relevance) && relevance >= 0 && relevance <= 3)) {
    ...
    sendJson(response, 200, { status: "saved", audit_id: auditId, labeled: state.labeled, total: state.total });
    return true;
  }
  ...
  if (request.method === "POST" && pathname === "/__human-audit/finalize") {
    ...
    fs.writeFileSync(FINAL_PATH, rows.map(row => JSON.stringify(row)).join("\n") + "\n", "utf8");
    ...
    sendJson(response, 200, {
      status: "finalized",
      rows: rows.length,
      output: "benchmark/multilingual-ab-delta-audit-v1.judgments.jsonl",
      finalizedAt,
    });
    return true;
  }
...
function serveStatic(response, pathname) {
  const requested = pathname === "/" ? "/benchmark/multilingual-ab-delta-audit.html" : pathname;
  let decoded;
...
server.listen(PORT, "127.0.0.1", () => {
  const state = statePayload();
  console.log();
  console.log("HELD-OUT MULTILINGUAL A/B DELTA AUDIT SERVER");
  console.log("============================================");
  console.log(`Progress: ${state.labeled}/${state.total}`);
  console.log(`Open: http://127.0.0.1:${PORT}/benchmark/multilingual-ab-delta-audit.html`);
  console.log();
  console.log("Blindness: A/B condition, ranks, providers, scores, and translation provenance are hidden.");

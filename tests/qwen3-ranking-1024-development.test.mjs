...
const manifestPath =
  "benchmark/qwen3/configs/qwen3-ranking-1024-development-v1.inference.json";

test("Qwen3 1024 development manifest changes only the intended inference boundary", async () => {
...
test("Qwen3 1024 development output and cache paths are isolated and clean", async () => {
  const manifest = JSON.parse(
    await readFile(manifestPath, "utf8"),
  );
  ...
  assert.equal(
    ...
    "benchmark/qwen3/cache/qwen3-ranking-1024-development-v1.cache.jsonl",
  );
...
test("Qwen3 1024 development command cannot overwrite or consume 4096 reference scores", async () => {
  ...
  const command =
    packageJson.scripts[
      "benchmark:qwen3:ranking:1024-development:infer"
    ];
  ...
  assert.match(
    command,
    /qwen3-ranking-1024-development-v1\.inference\.json/,
  );

  assert.match(
    command,
    /qwen3-ranking-1024-development-v1\.raw\.jsonl/,
  );

  assert.match(
    command,
    /qwen3-ranking-1024-development-v1\.cache\.jsonl/,
  );
